...
RSpec.feature "ListingFollowings" do 
  ...
  scenario "Logged-in user can't follow himself" do
    ...
    expect(page).not_to have_link("Follow", href: "/followings?user_id=#{@foo.id}")
   end
  ...
  scenario "Logged-in user can follow others" do
    ...
    link = "a[href='/followings?user_id=#{@bar.id}']"
    find(link).click
    
    expect(page).not_to have_link("Follow", href: "/followings?user_id=#{@bar.id}")
